...
根据对数似然函数的定义
$$
l(a) = \sum_{i=1}^{n}\log f(X_{i})
$$
在这个例子中我们可以直接用`dpois()`来代替密度函数 $f(x)$，然后使用 `log()` 取对数后求和，即可得到对数似然函数。
...
tmpF <- function(lambda, x){
  log_density_x <- log(dpois(x, lambda))  # 计算每个观测值上的密度的对数
  return (sum(log_density_x)) # 求和，即对数似然函数值
}
...
myF <- function(sigma, x){
  log_density_x <- log(dnorm(x, 0, sigma)) # 计算每个样本观测值密度的对数
  return(sum(log_density_x)) # 对所有密度的对数求和（即对数似然函数）
}
...
tmpF <- function(a, x){
  ...
  mu <- a[1]
  sigma <- a[2]
  log_density_x <- log(dnorm(x, mu, sigma))
  f <- sum(log_density_x)
  return(-f)
}
