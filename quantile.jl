...
# ╔═╡ 9237100c-afa6-11ef-1bd2-910124d80e38
using PlutoUI, Distributions, CairoMakie,LaTeXStrings

# ╔═╡ e57c7249-8f06-4e8a-940c-c43bf5068f43
md"
``\alpha``分位数的定义：对于随机变量``\xi``而言，其``\alpha``分位数``x_{\alpha}``满足：

$$F_{\xi}(x_{\alpha}) = \mathbb{P}(\xi \le x_{\alpha}) = \alpha$$
"

# ╔═╡ b95582c0-5f33-4455-b8a8-5795b353baf7
@bind alpha PlutoUI.Slider(0.01:0.01:0.99, show_value=true)

# ╔═╡ fcfbc4ef-65a1-413f-819a-6dee3e943623
begin
	x_alpha = quantile(Normal(), alpha)
	xx = -4:0.001:x_alpha
	f = Figure()
	ax = Axis(f[1,1],title="α = $(alpha)")
	b = band!(ax,xx, pdf.(Normal(), xx), zeros(length(xx)), alpha=0.5, color=:red)
	lines!(ax, -10:0.001:10,pdf.(Normal(),-10:0.001:10))
	hlines!(ax, 0, color=:black, linestyle=:dash)
	vlines!(ax, x_alpha, color=:black, linestyle=:dash)
	# lines!(ax, alpha.*sin.(-pi:0.001:pi))
	ylims!(ax, -0.1,0.5)
	xlims!(ax, -4, 4)
	text!(x_alpha+0.04,0,text=latexstring(L"x_{\alpha}="*"$(round(x_alpha,digits=2))") , align=(:left, :top), fontsize=20)
	hidedecorations!(ax, ticks=false, ticklabels=false)
	# hidespines!(ax)
	text!(x_alpha - 0.3, 0.05, text=L"\alpha",fontsize=30, align=[:right, :center])
	# axislegend(ax, [b], ["α"])
	f
end
...
manifest_format = "2.0"
project_hash = "622343a4d534addcf6b3aa913dd65b629bae71f4"
